...
export class TransactionButtonComponent implements OnInit {
  ...
  // @Input() public money: number; // Money in the wallet
  @Output() passEntry: EventEmitter<any> = new EventEmitter();
  inputQuantity: number = 0;
  purchasedQuantity: number = 0;
  money:number; // Money in the wallet
  stockItem;
  ...
  public executeOpt() {
    let money = parseFloat(localStorage.getItem('money') || '25000')
    if (this.opt === 'Sell') {
      ...
      money += this.inputQuantity * this.currentPrice;
      ...
      // let money = parseFloat(localStorage.getItem('money') || '25000')
      money -= this.stockItem.totalCost;
      localStorage.setItem('money',money.toString())
      console.log(
